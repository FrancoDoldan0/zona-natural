...
function applyBestOffer(price: number | null, offers: OfferLite[]) {
  if (price == null) return { priceOriginal: null as number | null, priceFinal: null as number | null, appliedOffer: null as OfferLite | null };
...
export async function GET(req: Request) {
  ...
  const items = await prisma.product.findMany({
    ...
    include: {
      ...
      images: { orderBy: [{ sortOrder: "asc" }, { id: "asc" }], select: { url: true, alt: true, sortOrder: true } },
    },
  });
  ...
  const offers = await prisma.offer.findMany({
    ...
    select: { id: true, title: true, discountType: true, discountVal: true, productId: true, categoryId: true },
