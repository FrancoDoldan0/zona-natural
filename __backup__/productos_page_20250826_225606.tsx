...
export default function ProductosPage() {
  ...
  const [sort, setSort] = useState<
    'final' | ' -final' | 'name' | ' -name' | 'price' | ' -price' | ' -id'
  >('final');
  ...
  const query = useMemo(() => {
    ...
    if (ids.length) {
      p.set('tagIds', ids.join(','));
    }
    const lo = num(minFinal);
    if (lo != null) p.set('minFinal', String(lo));
    const hi = num(maxFinal);
    if (hi != null) p.set('maxFinal', String(hi));
    return p.toString();
  ...
  return (
    <div
      style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 24, padding: '16px 20px' }}
    >
      {/* Filtros */}
      <aside style={{ border: '1px solid #eee', borderRadius: 12, padding: 16 }}>
        ...
        <div style={{ marginTop: 10 }}>
          ...
          <div
            style={{ display: 'grid', gap: 6, maxHeight: 260, overflow: 'auto', paddingRight: 4 }}
          >
            {tags.map((t) => (
              <label
                key={t.id}
        ...
        <div style={{ marginTop: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            ...
            <input
              placeholder="Máx."
              value={maxFinal}
              onChange={(e) => setMaxFinal(e.target.value)}
            />
          </div>
        </div>
