...
function toInt(s?: string | null, def = 1) {
  const n = parseInt(String(s ?? ''), 10);
  return Number.isFinite(n) && n > 0 ? n : def;
}

/** Normaliza a formato uruguayo internacional:
 *  - Acepta ya internacional: 598XXXXXXXX o 598XXXXXXXXX (8 o 9 dígitos)
 *  - Acepta nacional móvil/fijo: 09XXXXXXX (9), 9XXXXXXX (8), 2XXXXXXX (8), etc. -> antepone 598
 *  Devuelve null si no cumple longitudes válidas.
 */
function normalizeUy(raw?: string | null): string | null {
  ...
  // Fijos uruguayos suelen ser 8 dígitos; móviles 9 empezando en 9 (en nacional).
  if (noZero.length === 8 || noZero.length === 9) return '598' + noZero;

  return null;
}
function isUy(e164: string | null): e164 is string {
  return typeof e164 === 'string' && /^598\d{8,9}$/.test(e164);
}
...
export async function GET(req: NextRequest) {
  ...
  const dry = ['1', 'true', 'yes', 'on'].includes(
    (url.searchParams.get('dry') || '').toLowerCase(),
  );

  // Solo Uruguay: si viene ?phone=... se usa, si no, se usa WA_PHONE (también validado).
  const phoneRaw = url.searchParams.get('phone') || process.env.WA_PHONE || '';
  const phoneE164 = normalizeUy(phoneRaw);
  if (!isUy(phoneE164)) {
    const msg =
      'phone debe ser un número uruguayo en internacional: 598 + 8–9 dígitos (ej: 59891234567 o 59821234567).';
