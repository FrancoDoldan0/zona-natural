...
export default async function Page({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  const totalForView = Number.isFinite(filteredNum) ? filteredNum
  ...
  const filteredTotal = (data as any)?.filteredTotal;
  let totalForView = Number.isFinite(filteredTotal) ? Number(filteredTotal)
                    : (Number.isFinite((data as any)?.total) ? Number((data as any)?.total) : NaN);
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 12, justifyContent: "space-between", flexWrap: "wrap" }}>
        <div style={{ color: "#666" }}>{totalForView} resultados ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€š· PÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡gina {page} de {pageCount}</div>
