...
const paginaRe = (now: number, total: number) =>
  new RegExp(`PÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡gina\\s*${now}\\s*de\\s*${total}`, 'i');
...
async function getCounter(page: any, q: Record<string, any> = {}) {
  ...
  // Leer conteos desde el body (tolerante a 'PÃƒÂ¡gina X' o 'PÃƒÂ¡gina X de Y')
  const parsed = await page.evaluate(() => {
    const t = (document.body && (document.body.innerText || (document.body as any).textContent)) || '';
    const mPagina = t.match(/PÃƒÂ¡gina\s*(\d+)(?:\s*de\s*(\d+))?/i);
    const mTotal  = t.match(/(\d+)\s+resultados?/i);
...
test.describe('Productos ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ total filtrado, paginaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n y LCP priority', () => {
  ...
  test('2) Con filtros (perPage=1): contador y paginaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n usan el mismo totalForView', async ({ page }) => {
    const perPage = 1;
  ...
  test('3) Con filtros + perPage=1: "PÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡gina X de Y" refleja la navegaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n a page=2 cuando corresponde', async ({ page }) => {
    const perPage = 1;
  ...
  return paginaRe(1, 1).test(body);
      ...
      test.skip(true, 'Solo hay una pÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡gina con onSale=1; no aplica navegaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n a page=2.');
    }

  test('4) LCP: en pÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡gina 1 hay entre 1 y 3 imÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡genes sin loading="lazy"', async ({ page }) => {
  ...
  test('5) LCP: en pÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡gina 2 todas las imÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡genes son lazy (0 con prioridad)', async ({ page }) => {
