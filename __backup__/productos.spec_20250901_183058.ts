...
// Acepta "Página X de Y"
function paginaRe(now: number, total: number) {
  return new RegExp(`Página\\s*${now}\\s*de\\s*${total}`, 'i');
}

async function waitForCatalog(page: Page, timeout = 60_000) {
  await page.waitForLoadState('domcontentloaded');
  try {
    await page.waitForLoadState('networkidle', { timeout: Math.min(10_000, Math.floor(timeout / 2)) });
  } catch {}
 ...
 * Si recibe "q", navega manteniendo la ruta actual y aplicando esos query params.
...
async function getCounter(
  ...
  const pageNowSafe =
    Number.isFinite(parsed.pageNow) && parsed.pageNow > 0 ? parsed.pageNow : 1;
