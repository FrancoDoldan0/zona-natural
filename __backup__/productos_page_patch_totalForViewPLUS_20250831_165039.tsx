...
export default async function Page({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  const totalForView = Number.isFinite(filteredTotal) ? Number(filteredTotal) :
                       Number.isFinite(data.total) ? Number(data.total) :
                       items.length;
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 12, justifyContent: "space-between", flexWrap: "wrap" }}>
        <div style={{ color: "#666" }}>{totalForView} resultados Ã‚Â· PÃƒÂ¡gina {page} de {pageCount}</div>
