...
/** Deja solo dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­gitos */
 ...
 *  - Acepta ya internacional: 598XXXXXXXX o 598XXXXXXXXX (8 o 9 dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­gitos)
 *  - Acepta nacional mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³vil/fijo: 09XXXXXXX (9), 9XXXXXXX (8), 2XXXXXXX (8), etc. -> antepone 598
 *  Devuelve null si no cumple longitudes vÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡lidas.
 */
function normalizeUy(raw?: string | null): string | null {
  ...
  // Fijos uruguayos suelen ser 8 dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­gitos; mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³viles 9 empezando en 9 (en nacional).
  if (noZero.length === 8 || noZero.length === 9) return "598" + noZero;

  return null;
}
function isUy(e164: string | null): e164 is string { return typeof e164 === "string" && /^5989\d{7}$/.test(e164); }$/.test(e164); }$/.test(e164); }$/.test(e164); }$/.test(e164); }
...
export async function GET(req: NextRequest) {
  ...
  // Solo Uruguay: si viene ?phone=... se usa, si no, se usa WA_PHONE (tambiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©n validado).
  const phoneRaw = process.env.WA_PHONE || "";
  const phoneE164 = normalizeUy(phoneRaw);
  if (!isUy(phoneE164)) {
    const msg = "phone debe ser un nÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Âºmero uruguayo en internacional: 598 + 8ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ9 dÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­gitos (ej: 59891234567 o 59821234567).";
