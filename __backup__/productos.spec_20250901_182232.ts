...
const paginaRe = (now: number, total: number) =>
  new RegExp(`PÃƒÂ¡gina\\s*${now}\\s*de\\s*${total}`, 'i');
...
async function getCounter(page: any, q: Record<string, any> = {}) {
  ...
  // Fallbacks por si el contador aÃƒÂºn no se pintÃƒÂ³ completo
...
test.describe('Productos Ã¢â‚¬â€œ total filtrado, paginaciÃƒÂ³n y LCP priority', () => {
  ...
  return paginaRe(1, 1).test(body);
      ...
      test.skip(true, 'Solo hay una pÃƒÂ¡gina con onSale=1; no aplica navegaciÃƒÂ³n a page=2.');
    }
  });
