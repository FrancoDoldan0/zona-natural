...
export default async function Page({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  // Preferir filtrado si viene del backend; si no, total general; Ãºltimo recurso, cantidad de items.
...
return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 12, justifyContent: "space-between", flexWrap: "wrap" }}>
        <div style={{ color: '#666' }}>
  {totalForView} resultados · Página {page} de {Math.max(page, pageCount)}
</div>
        <div style={{ display: "flex", gap: 8 }}>
          {page > 1 && (
            <Link href={pageHref(searchParams, page - 1)} style={{ padding:"8px 12px", border:"1px solid #ddd", borderRadius:8 }}>
              ÃƒÂ¢Ã¢â‚¬Â Ã‚Â Anterior
            </Link>
          )}
          {page < pageCount && (
            <Link href={pageHref(searchParams, page + 1)} style={{ padding:"8px 12px", border:"1px solid #ddd", borderRadius:8 }}>
              Siguiente ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢
            </Link>
          )}
        </div>
      </div>
