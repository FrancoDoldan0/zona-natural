 ...
 *  - Acepta ya internacional: 598XXXXXXXX o 598XXXXXXXXX (8 o 9 dÃƒÂ­gitos)
 *  - Acepta nacional mÃƒÂ³vil/fijo: 09XXXXXXX (9), 9XXXXXXX (8), 2XXXXXXX (8), etc. -> antepone 598
 *  Devuelve null si no cumple longitudes vÃƒÂ¡lidas.
 */
function normalizeUy(raw?: string | null): string | null {
  ...
  // Fijos uruguayos suelen ser 8 dÃƒÂ­gitos; mÃƒÂ³viles 9 empezando en 9 (en nacional).
  if (noZero.length === 8 || noZero.length === 9) return "598" + noZero;

  return null;
}
function isUy(e164: string | null): e164 is string { return typeof e164 === "string" && /^5989\d{7}$/.test(e164); }$/.test(e164); }$/.test(e164); }
...
export async function GET(req: NextRequest) {
  ...
  // Solo Uruguay: si viene ?phone=... se usa, si no, se usa WA_PHONE (tambiÃƒÂ©n validado).
  const phoneRaw = process.env.WA_PHONE || "";
  const phoneE164 = normalizeUy(phoneRaw);
  if (!isUy(phoneE164)) {
    const msg = "phone debe ser un nÃƒÂºmero uruguayo en internacional: 598 + 8Ã¢â‚¬â€œ9 dÃƒÂ­gitos (ej: 59891234567 o 59821234567).";
