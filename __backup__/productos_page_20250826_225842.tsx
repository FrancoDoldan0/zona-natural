...
export async function generateMetadata(
  ...
  // No duplicar page/perPage en canonical
  qs.delete("page"); qs.delete("perPage");
  const canonical = `${siteUrl}/productos${qs.toString() ? `?${qs.toString()}` : ""}`;
  return {
    title: `Productos | ${siteName}`,
    description: "Descubrí productos naturales con ofertas y filtros por categoría, etiquetas y precio.",
    alternates: { canonical },
    // Si querés forzar index/noindex según filtros, podrías usar 'robots' aquí.
  };
}
...
async function fetchCatalog(searchParams: Record<string, string | string[] | undefined>) {
  ...
  const res = await fetch(`${base}/api/public/catalogo?${qs.toString()}`, { next: { revalidate: 120 } });
...
export default async function Page({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  // JSON-LD ItemList para los resultados de ESTA página
  const listLd = {
          ...
          return (
            <Link key={p.id} href={`/producto/${p.slug}`} style={{ textDecoration: "none", color: "inherit" }}>
              <div style={{ border: "1px solid #eee", borderRadius: 12, overflow: "hidden" }}>
                <div style={{ width: "100%", aspectRatio: "1/1", background: "#fafafa", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {p.cover
