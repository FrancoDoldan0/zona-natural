...
export default async function Page({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  // === ZN: total y pageCount consistentes con filtros ===
  const filterKeys = new Set(["onSale","minFinal","maxFinal","match","order","tagId","tagIds","q","search"]);
  const hasFilters = Object.entries(searchParams || {}).some(([k, v]) =>
    filterKeys.has(k) && (Array.isArray(v) ? v.length > 0 : String(v ?? "").trim() !== "")
  );
  ...
  // Preferir filtrado si viene del backend; si no, total general; ÃƒÆ’Ã‚Âºltimo recurso, cantidad de items.
...
return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 12, justifyContent: "space-between", flexWrap: "wrap" }}>
        <div style={{ color: '#666' }}>
  {ZN_VISIBLE_TOTAL} resultados Ã‚Â· PÃƒÂ¡gina {page} de {ZN_PAGECOUNT}
</div>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          {page < pageCount_ZN && (
            <Link href={pageHref(searchParams, page + 1)} style={{ padding:"8px 12px", border:"1px solid #ddd", borderRadius:8 }}>
              Siguiente ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢
            </Link>
          )}
        </div>
      </div>
