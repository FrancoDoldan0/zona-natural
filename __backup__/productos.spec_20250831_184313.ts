...
async function waitForCards(page, timeout = 60000) {
  ...
  // PequeÃ±o intento de networkidle (no crÃ­tico)
...
function paginaRe(p: number, n: number): RegExp {
  // Soporta "PÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡gina", "PÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡gina" y similares
...
async function getCounter(page: Page, _opts: { perPage?: number } = {}) {
  ...
  // PÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡gina P de N (tolerante)
...
test.describe('Productos ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ total filtrado, paginaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n y LCP priority', () => {
  ...
  test('3) Con filtros + perPage=1: "PÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡gina X de Y" refleja la navegaciÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n a page=2 cuando corresponde', async ({ page }) => {
    ...
    // PÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡gina 1 con filtros activos
    await page.goto(url('/productos', { onSale: 1, match: 'all', perPage }));
    await page.locator('a[href^="/producto/"]').first().waitFor({ timeout: 5000 }).catch(() => {});
