...
export type OfferInfo = { id:number; type:DiscountType; val:number; label:string; endAt: Date|null };
...
export async function bestOfferFor(productId: number, categoryId?: number|null, tagIds?: number[]){
  const now = new Date();
...
export async function computePriceForProduct(p: { id:number; price:number|null; categoryId:number|null; tags?: number[] }){
  if (p.price == null) return { priceOriginal: null as number|null, priceFinal: null as number|null, offer: null as OfferInfo|null };
  ...
  for (const o of offers){
    ...
    if (best === null || f < final){
      final = f;
      best = { id:o.id, type:o.discountType as DiscountType, val:Number(o.discountVal), label: labelFor(o.discountType as DiscountType, Number(o.discountVal)), endAt:o.endAt };
    }
  }
...
export async function computePricesBatch(products: BareProd[]){
  const now = new Date();
  const result = new Map<number, { priceOriginal:number|null; priceFinal:number|null; offer:OfferInfo|null }>();
  ...
  const offers = ors.length
    ? await prisma.offer.findMany({
        ...
        select: { id:true, discountType:true, discountVal:true, endAt:true, productId:true, categoryId:true, tagId:true },
      })
    : [];
    ...
    for (const p of products){
      ...
      if (cur.priceFinal == null || f < cur.priceFinal){
        result.set(p.id, {
          ...
          priceFinal: f,
          offer: { id:o.id, type:o.discountType as DiscountType, val:Number(o.discountVal), label: labelFor(o.discountType as DiscountType, Number(o.discountVal)), endAt:o.endAt }
        });
      }
    }
  }
