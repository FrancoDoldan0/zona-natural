...
if (!p) { console.error("Producto no encontrado:", productId); process.exit(2); }
...
const allOffers = await prisma.offer.findMany({
  ...
  orderBy: { id: "desc" },
  take: 20,
  select: { id:true,title:true,discountType:true,discountVal:true,productId:true,categoryId:true,tagId:true,startAt:true,endAt:true }
});
...
const activeOffers = await prisma.offer.findMany({
  ...
  orderBy: { id: "desc" },
  select: {
    id:true,title:true,discountType:true,discountVal:true,
    productId:true,categoryId:true,tagId:true,startAt:true,endAt:true
  },
});
...
// Mejor oferta
let final = price, best = null;
for (const o of activeOffers) {
  const f = apply(o.discountType, price, o.discountVal);
  if (best === null || (f ?? Infinity) < (final ?? Infinity)) { final = f; best = o; }
}
