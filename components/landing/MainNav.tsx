...
type Cat = {
  ...
  name: string;
  slug?: string;
  // tolerar distintos nombres que puede devolver el backend
  subcats?: Subcat[];
...
export default function MainNav() {
  ...
  // Cerrar al click fuera (mobile/desktop)
  useEffect(() => {
  ...
  return (
    <div className="w-full border-b border-emerald-100 bg-white">
      <div className="mx-auto max-w-7xl px-3 py-2 flex items-center gap-4 text-sm">
        {/* Wrapper para manejar hover/leave en desktop */}
        <div
          ref={wrapRef}
          className="relative"
        ...
        >
          ...
          {open && (
            <div
              className="absolute left-0 top-full mt-2 w-[min(92vw,820px)] z-40 rounded-xl border border-emerald-100 bg-white shadow-lg"
              role="menu"
            >
              <div className="grid grid-cols-1 md:grid-cols-3">
                ...
                {/* Columna subcategorías del activo */}
                <div className="md:col-span-2 border-t md:border-t-0 md:border-l border-emerald-100 p-3">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-1">
                    ...
                    {/* Fallback si no hay subcats: link directo a la categoría */}
                    {!!activeCat && !(activeCat.subcats?.length || activeCat.subcategories?.length || activeCat.children?.length) && (
                      <Link
                        href={`/catalogo?categoryId=${activeCat.id}`}
                        className="rounded-md px-3 py-2 hover:bg-emerald-50"
                      >
