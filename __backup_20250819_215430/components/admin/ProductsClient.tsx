...
function Row({ prod, onDone }: { prod: Prod; onDone: () => void }) {
  ...
  const [price, setPrice] = useState<string>(
    prod.priceCents != null ? (prod.priceCents / 100).toFixed(2) : '',
  );
  ...
  return (
    <tr className="border-t">
      ...
      <td className="p-2">
        <button className="rounded bg-gray-800 text-white px-3 py-1" onClick={save}>
          Guardar
        </button>
        <button className="ml-2 rounded bg-red-600 text-white px-3 py-1" onClick={remove}>
          Eliminar
        </button>
      </td>
    </tr>
