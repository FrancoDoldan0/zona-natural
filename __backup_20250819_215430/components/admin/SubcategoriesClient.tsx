...
export default function SubcategoriesClient({ initialCats, initialSubs }: { initialCats: Cat[]; initialSubs: Sub[] }) {
  const r = useRouter();
    ...
    <>
      <div className="flex items-center gap-2">
        ...
        <select
          ...
          value={selected}
          onChange={(e) => r.replace(e.target.value ? `/admin/subcategorias?categoryId=${e.target.value}` : `/admin/subcategorias`)}
        >
      ...
      <form
        onSubmit={async (e) => {
          ...
          if (res.ok) { setName(""); setSlug(""); r.refresh(); }
        }}
      ...
      >
        ...
        <label className="block">
          ...
          <input className="border rounded px-3 py-2" value={slug} onChange={e=>setSlug(slugify(e.target.value))} />
        </label>
        <label className="block">
