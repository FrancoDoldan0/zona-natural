...
function sniffMagic(buf: Uint8Array){
  ...
  // JPEG
  if (buf.length>=3 && b(0)===0xFF && b(1)===0xD8 && b(2)===0xFF) return {ext:".jpg", mime:"image/jpeg"};
  // PNG
  if (buf.length>=8 && [0x89,0x50,0x4E,0x47,0x0D,0x0A,0x1A,0x0A].every((v,i)=>b(i)===v)) return {ext:".png", mime:"image/png"};
  // WebP: "RIFF....WEBP"
  if (buf.length>=12 && ascii(0,4)==="RIFF" && ascii(8,4)==="WEBP") return {ext:".webp", mime:"image/webp"};
  // AVIF: ftyp.... "avif"/"avis"
  if (buf.length>=12 && ascii(4,4)==="ftyp" && ["avif","avis"].includes(ascii(8,4))) return {ext:".avif", mime:"image/avif"};
  return null;
}
...
export async function POST(req: Request){
  ...
  if (!rl.ok) return NextResponse.json({ ok:false, error:"Too many uploads, try later." }, { status:429 });
