...
export async function GET(_req: NextRequest, ctx:{ params:{ id:string } }){
  ...
  const item = await prisma.product.findUnique({
    where:{ id },
    include:{ category:true, subcategory:true, images:{ orderBy:{ sortOrder:"asc" } } }
  });
...
export async function PUT(req: NextRequest, ctx:{ params:{ id:string } }){
  try{
    ...
    if ("slug" in b) data.slug = b.slug || slugify(b.name||"");
    ...
    if (e?.code === "P2002") {
      return NextResponse.json({ ok:false, error:"unique_constraint", field:"slug" }, { status:409 });
    }
...
export async function DELETE(req: NextRequest, ctx:{ params:{ id:string } }){
  try{
    ...
    if (imgCount > 0) {
      return NextResponse.json({ ok:false, error:"has_images", detail:`Tiene ${imgCount} imágenes. Eliminá primero las imágenes.` }, { status:400 });
    }
