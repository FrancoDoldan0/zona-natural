...
export async function GET(req: NextRequest){
  ...
  const [total, items] = await Promise.all([
    ...
    prisma.product.findMany({
      where, skip, take: perPage, orderBy:{ id:"desc" },
      include: {
...
export async function POST(req: NextRequest){
  try{
    ...
    if(!parsed.success){
      return NextResponse.json({ ok:false, error:"validation_failed", detail: parsed.error.format() }, { status:400 });
    }
    const b = parsed.data;
    ...
    await audit(req, "CREATE", "Product", created.id, { name: b.name, slug });
