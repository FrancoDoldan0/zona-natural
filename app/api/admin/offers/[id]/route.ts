...
export async function PUT(req: Request, { params }:{ params:{ id:string } }){
  try{
    const id = Number(params.id); if(!Number.isFinite(id)) return NextResponse.json({ ok:false, error:"invalid_id" }, { status:400 });
    ...
    return NextResponse.json({ ok:false, error:"admin_offers_put_failed", detail: e?.message ?? String(e) }, { status:500 });
  }
}
...
export async function DELETE(req: Request, { params }:{ params:{ id:string } }){
  try{
    ...
    return NextResponse.json({ ok:false, error:"admin_offers_delete_failed", detail: e?.message ?? String(e) }, { status:500 });
  }
}