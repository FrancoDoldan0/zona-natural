...
export async function GET(_req: NextRequest, ctx:{ params:{ slug:string } }){
  ...
  const { priceOriginal, priceFinal, offer } =
    await computePriceForProduct({ id:p.id, price:p.price, categoryId:p.categoryId, tags });
  ...
  const breadcrumbs = [
    ...
    p.subcategory ? { label: p.subcategory.name, href:`/c/${p.category?.slug}/${p.subcategory.slug}` } : null,
  ...
  return json({
    ...
    item:{
      ...
      priceOriginal, priceFinal, offer,
