...
export async function GET(req: NextRequest){
  ...
  if (tagIds.length){
    if (match === "all"){
      ...
      where.AND = (where.AND || []).concat(tagIds.map(id => ({ productTags: { some: { tagId: id } } })));
    } else {
  ...
  const bare = itemsRaw.map((p:any)=>({
    id: p.id, price: p.price, categoryId: p.categoryId, tags: (p.productTags||[]).map((t:any)=>t.tagId)
  }));
  ...
  let items = itemsRaw.map((p:any)=>{
    ...
    const hasDiscount =
      pr.priceOriginal != null &&
      pr.priceFinal != null &&
      pr.priceFinal < pr.priceOriginal;
  ...
  if (sort === "final") {
    items.sort((a:any,b:any)=> (a.priceFinal??Number.POSITIVE_INFINITY) - (b.priceFinal??Number.POSITIVE_INFINITY));
  } else if (sort === "-final") {
    items.sort((a:any,b:any)=> (b.priceFinal??Number.NEGATIVE_INFINITY) - (a.priceFinal??Number.NEGATIVE_INFINITY));
  }
  ...
  return json({
    ...
    filteredPageCount,
    appliedFilters: {
      q, categoryId: Number.isFinite(categoryId)?categoryId:null,
