...
export default function OffersPage(){
  ...
  const [prodQ, setProdQ] = useState(""); const [prodSel, setProdSel] = useState<ProductOpt|null>(null);
  const [catQ, setCatQ] = useState("");  const [catSel, setCatSel] = useState<CategoryOpt|null>(null);
  ...
  useEffect(()=>{
    const t = setTimeout(async ()=>{
      ...
      const r = await fetch(url); const j = await r.json(); if (j.ok) setProdOpts(j.items);
    }, 250);
  ...
  useEffect(()=>{
    const t = setTimeout(async ()=>{
      ...
      const r = await fetch(url); const j = await r.json(); if (j.ok) setCatOpts(j.items);
    }, 250);
    return ()=>clearTimeout(t);
  },[catQ]);
  ...
  async function onCreate(e:React.FormEvent){
    e.preventDefault();
    const payload:any = {
      title, description: description.trim()||null,
      discountType: dtype, discountVal: Number(dval||0),
    ...
    if (j.ok) {
      setTitle(""); setDescription(""); setDtype("PERCENT"); setDval("10");
      setTarget("general"); setProdSel(null); setCatSel(null);
      setStartAt(""); setEndAt(""); load();
  ...
  const preview = useMemo(()=>{
    ...
    const tgt = target==="general" ? "General"
              : target==="product"  ? (prodSel ? `Producto: ${prodSel.name}` : "Producto (sin seleccionar)")
              : target==="category" ? (catSel ? `Categoría: ${catSel.name}` : "Categoría (sin seleccionar)")
              : "";
    return `${val} · ${tgt}`;
  ...
  return (
    <main className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <form onSubmit={onCreate} className="grid gap-2 md:grid-cols-6 border rounded p-4">
        <input className="border rounded p-2 md:col-span-3" placeholder="Título" value={title} onChange={e=>setTitle(e.target.value)} />
        <select className="border rounded p-2" value={dtype} onChange={e=>setDtype(e.target.value as any)}>
          <option value="PERCENT">% Porcentaje</option>
          <option value="AMOUNT">$ Monto</option>
        </select>
        <input className="border rounded p-2" type="number" step="0.01" min="0" value={dval} onChange={e=>setDval(e.target.value)} />
        <select className="border rounded p-2" value={target} onChange={e=>{ setTarget(e.target.value as any); setProdSel(null); setCatSel(null); }}>
          <option value="general">General</option>
          <option value="product">Producto</option>
          <option value="category">Categoría</option>
        </select>

        <textarea className="border rounded p-2 md:col-span-6" placeholder="Descripción (opcional)"
          rows={2} value={description} onChange={e=>setDescription(e.target.value)} />

        {target==="product" && (
          <div className="md:col-span-6 border rounded p-3 space-y-2">
            <div className="flex gap-2 items-center">
              <input className="border rounded p-2 flex-1" placeholder="Buscar producto por nombre/slug/SKU"
                value={prodQ} onChange={e=>setProdQ(e.target.value)} />
              {prodSel && <button type="button" className="border rounded px-2" onClick={()=>setProdSel(null)}>Quitar</button>}
            </div>
            {!prodSel && (
              <ul className="grid md:grid-cols-3 gap-2">
                {prodOpts.map(p=>(
                  <li key={p.id}>
                    <button type="button" className="border rounded px-2 py-1 w-full text-left"
                      onClick={()=>setProdSel(p)}>{p.name} <span className="opacity-60">({p.slug})</span></button>
                  </li>
                ))}
        ...
        {target==="category" && (
          <div className="md:col-span-6 border rounded p-3 space-y-2">
            <div className="flex gap-2 items-center">
              <input className="border rounded p-2 flex-1" placeholder="Buscar categoría"
                value={catQ} onChange={e=>setCatQ(e.target.value)} />
              {catSel && <button type="button" className="border rounded px-2" onClick={()=>setCatSel(null)}>Quitar</button>}
            </div>
            {!catSel && (
              <ul className="grid md:grid-cols-3 gap-2">
                {catOpts.map(c=>(
                  <li key={c.id}>
                    <button type="button" className="border rounded px-2 py-1 w-full text-left"
                      onClick={()=>setCatSel(c)}>{c.name} <span className="opacity-60">({c.slug})</span></button>
                  </li>
                ))}
        ...
        <button className="border rounded px-4 py-2 md:col-span-6 w-fit" type="submit">Crear</button>
      </form>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm border rounded">
          ...
          <tbody>
            {items.map(o=>{
              const val = o.discountType==="PERCENT" ? `${o.discountVal}%` : `$ ${o.discountVal.toFixed(2)}`;
              const tgt = o.product ? `Producto: ${o.product.name}` :
            ...
            {!items.length && <tr><td className="p-3 opacity-70" colSpan={7}>Sin ofertas</td></tr>}
          </tbody>
        </table>
      </div>

      <div className="text-sm opacity-70">
        Tip: las ofertas públicas están en <Link href="/api/public/offers" className="underline">/api/public/offers</Link>
      </div>
    </main>
  );
}